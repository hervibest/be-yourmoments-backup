...
func NewPaymentAdapter(midtransClient *config.MidtransClient, cacheAdapter CacheAdapter, logs *logger.Log) PaymentAdapter {
...
func (a *paymentAdapter) CheckTransactionStatus(ctx context.Context, transactionId string) (*coreapi.TransactionStatusResponse, error) {
	...
	if err != nil {
		a.logs.CustomError("[PaymentAdapter] CheckTransactionStatus error: %v", err)
